...
public class astrocde
{
	...
	public static int	wowBaseFrequency;		/* Some games (Qbert) change this */
	public static int 	wowBaseVolume;
	public static int 	wowChannel = 0;
	public static GameSamples wowSamples;
	...
	public static String wowWordTable[] =
	{
	...
	public static int num_samples = wowWordTable.length;
	...
	/* Phoneme to word translation */
		...
		if ((totalword.length()) == 0) {
		   totalword=PhonemeTable[Phoneme];	                   /* Copy over the first phoneme */
		   if (plural != 0) {
	...
	//	logerror("Total word = %s\n",totalword);
		...
		for (i=0; wowWordTable[i] != null; i++) {
		   if (!(wowWordTable[i].equals(totalword))) {		   /* Scan the word (sample) table for the complete word */
		  /* WOW has Dungeon */
			  if ((!("GDTO1RFYA2N".equals(totalword))) || (!("RO1U1BAH1T".equals(totalword))) || (!("KO1UH3I3E1N".equals(totalword)))) {		   /* May be plural */
				 plural=i+1;
				 oldword=totalword;
	...
	public static int wow_status_r()
	{
	//	logerror("asked for samples status %d\n",wowChannel);
		return sample_playing(wowChannel)!=0?0:1;
	}
