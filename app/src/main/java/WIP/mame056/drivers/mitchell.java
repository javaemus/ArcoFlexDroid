...
import static WIP.mame056.vidhrdw.mitchell.*;
...
public class mitchell
{
	...
	***************************************************************************/
	...
	static nvramPtr nvram_handler = new nvramPtr() {
                ...
                if (read_or_write != 0)
		{
			...
			if (nvram_size != null)	/* Super Pang, Block Block */
				osd_fwrite(file,nvram,nvram_size[0]);	/* NVRAM */
		}
		else
		{
			...
			if (file != null)
			{
				...
				if (nvram_size != null)	/* Super Pang, Block Block */
					osd_fread(file,nvram,nvram_size[0]);	/* NVRAM */
			}
			else
	...
	public static ReadHandlerPtr input_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		switch (input_type)
		{
			...
			case 1:	/* Mahjong games */
				if (offset!=0) return mahjong_input_r.handler(offset-1);
				else return readinputport(1);
				//break;
			case 2:	/* Block Block - dial control */
				if (offset!=0) return block_input_r.handler(offset-1);
				else return readinputport(1);
				//break;
			case 3:	/* Super Pang - simulate START 1 press to initialize EEPROM */
				if (offset!=0 || init_eeprom_count == 0) return readinputport(1 + offset);
				else
				{
					init_eeprom_count--;
	...
	public static WriteHandlerPtr input_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		switch (input_type)
		{
			...
			default:
				...
				break;
			case 1:
				mahjong_input_select_w.handler(offset,data);
				break;
			case 2:
	...
	***************************************************************************/
	...
	public static InitDriverPtr init_dokaben = new InitDriverPtr() { public void handler()
	{
		input_type = 0;
		nvram_size[0] = 0;
		mgakuen2_decode();
	...
	public static InitDriverPtr init_pangb = new InitDriverPtr() { public void handler()
	{
		input_type = 0;
		nvram_size[0] = 0;
		bootleg_decode();
	} };
	public static InitDriverPtr init_cworld = new InitDriverPtr() { public void handler()
	{
		input_type = 0;
		nvram_size[0] = 0;
		cworld_decode();
	} };
	public static InitDriverPtr init_hatena = new InitDriverPtr() { public void handler()
	{
		input_type = 0;
		nvram_size[0] = 0;
		hatena_decode();
	} };
	public static InitDriverPtr init_spang = new InitDriverPtr() { public void handler()
	{
		input_type = 3;
		nvram_size[0] = 0x80;
	...
	public static InitDriverPtr init_qsangoku = new InitDriverPtr() { public void handler()
	{
		input_type = 0;
		nvram_size[0] = 0;
		qsangoku_decode();
	} };
	public static InitDriverPtr init_mgakuen = new InitDriverPtr() { public void handler()
	{
		input_type = 1;
	} };
	...
	public static InitDriverPtr init_marukin = new InitDriverPtr() { public void handler()
	{
		input_type = 1;
		nvram_size[0] = 0;
		marukin_decode();
	} };
	public static InitDriverPtr init_block = new InitDriverPtr() { public void handler()
	{
		input_type = 2;
		nvram_size[0] = 0x80;
