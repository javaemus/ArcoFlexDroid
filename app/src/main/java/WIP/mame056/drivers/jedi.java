...
public class jedi
{
	...
	/* constants */
	public static int MAIN_CPU_OSC = 10000000;
	public static int SOUND_CPU_OSC = 12096000;
	
	
	/* local variables */
	static int control_num;
	static int sound_latch;
	static int sound_ack_latch;
	static int sound_comm_stat;
	static int speech_write_buffer;
	static int speech_strobe_state;
	static UBytePtr nvram = new UBytePtr();
	static int[] nvram_size = new int[2];
	static int nvram_enabled;
	...
	static timer_callback generate_interrupt = new timer_callback() {
            public void handler(int scanline) {
                /* IRQ is set by /32V */
		cpu_set_irq_line(0, M6502_INT_IRQ, (scanline & 32) != 0 ? CLEAR_LINE : ASSERT_LINE);
		cpu_set_irq_line(1, M6502_INT_IRQ, (scanline & 32) != 0 ? CLEAR_LINE : ASSERT_LINE);
	...
	static timer_callback delayed_sound_latch_w = new timer_callback() {
	...
	{
		coin_counter_w.handler(offset, data >> 7);
	} };
