...
public class kabuki
{
	...
	static void kabuki_decode(UBytePtr src, UBytePtr dest_op, UBytePtr dest_data,
	...
	static void cps1_decode(int swap_key1,int swap_key2,int addr_key,int xor_key)
	{
		...
		UBytePtr backup = new UBytePtr(memory_region(REGION_USER1));
		...
		if (backup != null) memcpy(backup,rom,0x8000);
	
		memory_set_opcode_base(1, new UBytePtr(rom, diff));
		kabuki_decode(rom, new UBytePtr(rom, diff), rom, 0x0000,0x8000, swap_key1,swap_key2,addr_key,xor_key);
	}
	
	static void wof_decode()      { cps1_decode(0x01234567,0x54163072,0x5151,0x51); }
	static void dino_decode()     { cps1_decode(0x76543210,0x24601357,0x4343,0x43); }
	static void punisher_decode() { cps1_decode(0x67452103,0x75316024,0x2222,0x22); }
	static void slammast_decode() { cps1_decode(0x54321076,0x65432107,0x3131,0x19); }
}
