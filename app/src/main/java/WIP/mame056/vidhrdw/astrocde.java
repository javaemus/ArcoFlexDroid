...
import static mame056.cpu.z80.z80H.Z80_AF;
...
public class astrocde
{
	...
	public static int MAX_LINES = 204;
	
	public static int MAX_INT_PER_FRAME = 256;
	 ...
	 */
	public static int CLOCKS_PER_LINE = 455;
	...
	/* This value affects the star blinking and the sparkle patterns.
	   ...
	   not been verified.
	 */
	public static int CLOCKS_PER_FRAME = (CLOCKS_PER_LINE*262);
	
	public static int RNG_PERIOD = 131071;	/* 2^17-1 */
	static int[] rng = new int[RNG_PERIOD];
	static int[] star= new int[CLOCKS_PER_LINE-16+SCREEN_WIDTH*MAX_LINES];
	...
	static int[][] sparkle = new int[MAX_INT_PER_FRAME][4];	/* sparkle[line][0] is star enable */
	...
	public static VhConvertColorPromPtr astrocde_init_palette = new VhConvertColorPromPtr() { public void handler(char []palette, char []colortable, UBytePtr color_prom) 
	{
		...
		for(i=0;i<32;i++)
		{
			RY = (float) (C*Math.sin(i*2.0*3.14159/32.0));
			if (i == 0)
				BY = 0;
			else
				BY = (float) (C*Math.cos(i*2.0*3.14159/32.0));
	
			for(j=0;j<8;j++)
			{
				Y = (float) ((j/7.0)*(brightest-dimmest)+dimmest);
	
				/* Transform to RGB */
	
				R = (RY+Y)*255;
				G = (float) ((Y - 0.299*(RY+Y) - 0.114*(BY+Y))/0.587*255);
				B = (BY+Y)*255;
				...
				palette[_palette] = (char) Math.floor(R+.5);
				_palette++;
				palette[_palette] = (char) Math.floor(G+.5);
				_palette++;
				palette[_palette] = (char) Math.floor(B+.5);
				_palette++;
			}
		}
	} };
...
/*TODO*///	#endif
			...
			for (i = 0; i <= loops;i++)
			{
				for (j = 0;j <= length;j++)
				{
					if ((mode & 0x08)==0 || j < length)
					{
						if ((mode & 0x01) != 0)			/* Direction */
							...
							if (dest >= 0) cpu_writemem16(dest,RAM.read(src));	/* ASG 971005 */
					}
				...
				dest += skip;	/* extend the sign of the skip register */
...
/*TODO*///					if (mode & 0x10)
/*TODO*///					{
/*TODO*///						if (lo < 0x100) hi -= 0x100;
/*TODO*///					}
/*TODO*///					else
/*TODO*///					{
/*TODO*///						if (lo > 0xff) hi += 0x100;
/*TODO*///					}
/*TODO*///					dest = hi | (lo & 0xff);
/*TODO*///				}
/*TODO*///	#endif
			}
		}
	...
	public static InitDriverPtr init_star_field = new InitDriverPtr() { public void handler()
	{
		int generator;
		int count,x,y;
	...
	public static ReadHandlerPtr gorf_io_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		switch (offset)
		{
			case 0: coin_counter_w.handler(0,data); break;
			case 1: coin_counter_w.handler(1,data); break;
...
/*TODO*///	#endif
	...
	public static ReadHandlerPtr wow_io_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		switch (offset)
		{
			...
			case 4: sparkle[CurrentScan][2] = data; break;
			case 5: sparkle[CurrentScan][3] = data; break;
			case 7: coin_counter_w.handler(2,data); break;
		}
...
/*TODO*///	#endif
	...
	{
		rng = new int[RNG_PERIOD];
		star = new int[SCREEN_WIDTH * MAX_LINES];
	
		if (rng==null || star==null)
			return 1;
	
		/*TODO*///memset(sparkle,0,sparkle.length);
		CurrentScan = 0;
	...
	public static void wow_update_line(mame_bitmap bitmap,int line)
	{
		...
		/*TODO*///rngoffs = MOD_U32_U64_U32( MUL_U64_U32_U32(
                rngoffs = ((cpu_getcurrentframe() % RNG_PERIOD * CLOCKS_PER_FRAME) % RNG_PERIOD);
