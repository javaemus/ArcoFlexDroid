...
public class vigilant
{
	...
	***************************************************************************/
		...
		for (offs = 0; offs<videoram_size[0]; offs+=2 )
		{
			...
			else	 /* background */
			{
				...
				drawgfx(bitmap,Machine.gfx[0],
						...
						0,0,
						sx,sy,
						Machine.visible_area,(opaque!=0 || color >= 8) ? TRANSPARENCY_NONE : TRANSPARENCY_PEN,0);
			}
		}
	}
	...
	static void draw_background( mame_bitmap bitmap )
	{
		...
		copyscrollbitmap(bitmap,bg_bitmap,1,new int[]{scrollx},0,new int[]{0},bottomvisiblearea,TRANSPARENCY_NONE,0);
	}
	...
	***************************************************************************/
	...
	static void draw_sprites(mame_bitmap bitmap, rectangle clip)
	{
		int offs;
