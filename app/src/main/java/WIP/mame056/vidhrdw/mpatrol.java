...
{
	public static int BGHEIGHT = (64);
	
	static int[] scrollreg=new int[16];
	static int bg1xpos,bg1ypos,bg2xpos,bg2ypos,bgcontrol;
	static mame_bitmap[] bgbitmap=new mame_bitmap[3];
        
        static {
            for (int i=0 ; i<3 ; i++)
                bgbitmap[i] = new mame_bitmap();
        }
	...
	***************************************************************************/
	...
	public static VhConvertColorPromPtr mpatrol_vh_convert_color_prom = new VhConvertColorPromPtr() {
		...
		{
			COLOR(colortable,1,i,128+32 + (color_prom.read()));
                        color_prom.inc();
			if (i % 4 == 3) color_prom.inc(4);	/* half of the PROM is unused */
		}
	...
	***************************************************************************/
	...
	static void draw_background(mame_bitmap bitmap,
								...
								int transparency)
	{
		rectangle clip1=new rectangle(), clip2=new rectangle();
	...
	public static VhUpdatePtr mpatrol_vh_screenrefresh = new VhUpdatePtr() { public void handler(mame_bitmap bitmap,int full_refresh) 
	{
		...
		/* copy the temporary bitmap to the screen */
		{
			...
			if (flip_screen() != 0)
			{
				...
				copyscrollbitmap(bitmap,tmpbitmap,32,scroll,0,new int[]{0},clip,TRANSPARENCY_COLOR,0);
			}
			else
			{
				clip.min_y = 0;
