...
public class snk
{
	...
	*******************************************************************************/
	...
	public static void print(mame_bitmap bitmap, int num, int row ){
	...
	public static void tnk3_draw_text(mame_bitmap bitmap, int bank, UBytePtr source ){
    ...
    public static void ikari_draw_background(mame_bitmap bitmap, int xscroll, int yscroll ){
            GfxElement gfx = Machine.gfx[GFX_TILES];
	...
	shadows_visible = shadows_visible!=0?0:1;
...
/**************************************************************/
    ...
    static void tdfever_draw_background(mame_bitmap bitmap,
    ...
    public static VhUpdatePtr tdfever_vh_screenrefresh = new VhUpdatePtr() { public void handler(mame_bitmap bitmap,int full_refresh) 
     {
	UBytePtr ram = memory_region(REGION_CPU1);
	shadows_visible = shadows_visible!=0?0:1;
    ...
    public static VhUpdatePtr ftsoccer_vh_screenrefresh = new VhUpdatePtr() { public void handler(mame_bitmap bitmap,int full_refresh) 
    {
	UBytePtr ram = memory_region(REGION_CPU1);
	shadows_visible = shadows_visible!=0?0:1;
	{
    ...
    public static void gwar_draw_sprites_32x32(mame_bitmap bitmap, int xscroll, int yscroll ){
	...
	public static VhUpdatePtr sgladiat_vh_screenrefresh = new VhUpdatePtr() { public void handler(mame_bitmap bitmap,int full_refresh) 
	{
		...
		attributes = pMem.read(0xd300);
	
		scrolly =  -8+pMem.read(0xd600);
		scrollx = -16+pMem.read(0xd700) + ((attributes&0x02)!=0?256:0);
		sgladiat_draw_background( bitmap, -scrollx, -scrolly );
	
		scrolly =  8+pMem.read(0xd400);
		scrollx = 30+pMem.read(0xd500) + ((attributes&0x01)!=0?256:0);
		tnk3_draw_sprites( bitmap, scrollx, scrolly );
	
		tnk3_draw_text(bitmap, 0, new UBytePtr(pMem, 0xf000) );
	} };
