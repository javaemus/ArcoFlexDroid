	...
	static void draw_background( mame_bitmap bitmap )
	{
	...
	
		{
			...
			if (flipscreen != 0)
			{
				...
				/* scroll (wraps around) */
				for (offs = 64;offs < 128;offs++) xscroll[255-offs] = troangel_scroll.read(64);
	
				/* linescroll (no wrap) */
				for (offs = 128;offs < 256;offs++) xscroll[255-offs] = troangel_scroll.read(offs);
			}
			else
			{
				/* fixed */
				...
				/* scroll (wraps around) */
				for (offs = 64;offs < 128;offs++) xscroll[offs] = -troangel_scroll.read(64);
	
				/* linescroll (no wrap) */
				for (offs = 128;offs < 256;offs++) xscroll[offs] = -troangel_scroll.read(offs);
			}
	
			copyscrollbitmap(bitmap,tmpbitmap,256,xscroll,0,new int[]{0},Machine.visible_area,TRANSPARENCY_NONE,0);
		}
	}
