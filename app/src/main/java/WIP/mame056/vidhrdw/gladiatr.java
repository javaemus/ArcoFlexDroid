...
public class gladiatr
{
	...
	static void render_text( mame_bitmap bitmap ){
		...
		if( base_scroll < 0xd0 ){ /* panning text */
			...
			dx = 0;
			if( (video_attributes&0x08)==0 ) source.inc( 32 ); /* page 2 */
		}
	...
	static void draw_sprite( mame_bitmap bitmap, int tile_number, int color, int sx, int sy, int xflip, int yflip, int big ){
		rectangle clip = Machine.visible_area;
	...
	static void render_sprites(mame_bitmap bitmap){
		UBytePtr source = new UBytePtr(spriteram);
		UBytePtr finish = new UBytePtr(source, 0x400);
