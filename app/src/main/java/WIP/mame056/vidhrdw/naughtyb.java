	...
	***************************************************************************/
	...
	public static VhConvertColorPromPtr naughtyb_vh_convert_color_prom = new VhConvertColorPromPtr() {
		...
		for (i = 0;i < 8;i++)
		{
			...
			for (j = 0;j < 4;j++)
			{
				COLOR(colortable,0,4*i + j*4*8,i + j*64);
				COLOR(colortable,0,4*i + j*4*8 + 1,8 + i + j*64);
				COLOR(colortable,0,4*i + j*4*8 + 2,2*8 + i + j*64);
				COLOR(colortable,0,4*i + j*4*8 + 3,3*8 + i + j*64);
			}
		}
		...
		for (i = 0;i < 8;i++)
		{
			...
			for (j = 0;j < 4;j++)
			{
				COLOR(colortable,1,4*i + j*4*8,i + 32 + j*64);
				COLOR(colortable,1,4*i + j*4*8 + 1,8 + i + 32 + j*64);
				COLOR(colortable,1,4*i + j*4*8 + 2,2*8 + i + 32 + j*64);
				COLOR(colortable,1,4*i + j*4*8 + 3,3*8 + i + 32 + j*64);
			}
		}
            }
        };
	...
	***************************************************************************/
	public static VhUpdatePtr naughtyb_vh_screenrefresh = new VhUpdatePtr() { public void handler(mame_bitmap bitmap,int full_refresh) 
	{
		...
		/* copy the temporary bitmap to the screen */
		{
			...
			copyscrollbitmap(bitmap,tmpbitmap,1,new int[]{scrollx},0,new int[]{0},scrollvisiblearea,TRANSPARENCY_NONE,0);
		}
	} };
}
