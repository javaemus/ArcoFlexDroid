...
public class GfxManager implements VideoConstants {
   ...
   protected final BitMap decode(int tile, int col, BitMap tileImg, int rot, int flip, int trans, int transcol) {
      ...
      for (int y=0; y<=h; y++) {
         int oy = gdi.gfx.offsY[y];
         for (int x=0; x<=w; x++) {
            int ox = gdi.gfx.offsX[x];
            byteOffset= (ox)&0xfffff8;  // offset within the character in bytes of this pixel
            bitOffset = (ox)&7; // offset within the character in bits of this pixel
            realXOffset=byteOffset+(tileMameOff-bitOffset);
            colorIndex=0;

            try {
               for (int p=0; p < gdi.gfx.planes; p++) {  // This loop assembles the colorindex
                  int tmp = realXOffset + oy + gdi.gfx.offsPlane[p];
                  byteOffset = (tmp) >> 3;
                  bitOffset  = (tmp) & 7;
                  if ( (GFX[charAddress+byteOffset] & (1<<bitOffset)) != 0) {
