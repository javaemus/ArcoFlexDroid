...
public class retofinv
{
	...
	public static VhConvertColorPromPtr retofinv_vh_convert_color_prom = new VhConvertColorPromPtr() {
		...
		for(i = 0;i < TOTAL_COLORS(2);i++){
			COLOR(colortable,2,i, adj_data(color_prom.read()));
                        color_prom.inc();
                }
		...
		for(i = 0;i < TOTAL_COLORS(1);i++)
			COLOR(colortable,1,i, adj_data(color_prom.read(i)));
            }
        };
	
	public static VhStartPtr retofinv_vh_start = new VhStartPtr() { public int handler() 
	{
		if ((bg_dirtybuffer = new UBytePtr(retofinv_videoram_size[0])) == null)
		{
			return 1;
		}
	...
	public static void retofinv_draw_background(mame_bitmap bitmap)
	{
		...
		/* save active bank */
		bg_bank = (retofinv_bg_char_bank.read(0) & 1) != 0 ? 1 : 0;
		...
		for (y = 31; y>=0; y--)
		{
			for (x = 31; x>=0; x--)
			{
				...
				if ((bg_dirtybank!=0) || (bg_dirtybuffer.read(offs) != 0))
				{
					sx = 31-x;
					sy = 31-y;
